...
class deviceController extends mainModel{
                    ...
                    <tr class="bg-white border-b border-gray-200 hover:bg-gray-200">
                        ...
                        <td class="px-6 py-3 text-center">
                            <div class="flex justify-center items-center">
                                <a href="' . APPURL . 'withdrawDevice/' . $rows['device_ID'] . '/" class="bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300  rounded-full text-base p-2.5 text-center inline-flex items-center me-2 transition duration-100">
                                <svg class="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            if ($_SESSION['role'] == 1) {
                                $table .= '<form class="AjaxForm" action="' . APPURL . 'app/ajax/deviceAjax.php" method="POST">
    ...
    public function withdrawDeviceListController($page, $register, $url, $search){
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                            <thead class="text-base text-white uppercase bg-gray-800">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Articulo</th>
                                    <th scope="col" class="px-6 py-3">Serial</th>
                                    <th scope="col" class="px-6 py-3">Fecha de Retiro</th>
                                    <th scope="col" class="px-6 py-3 text-center">Ver</th>
                                    <th scope="col" class="px-6 py-3">
                                        <span class="sr-only">Edit</span>
                                    </th>
            ...
            foreach ($data as $rows) {
                $table .= '
                    <tr class="bg-white border-b border-gray-200 hover:bg-gray-200">
                        ...
                        <td class="px-6 py-3 text-center">
                            <div class="flex justify-center items-center">
                            <a href="' . APPURL . 'updateWithdrawDevices/' . $rows['device_ID'] . '/" class="bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-full text-base p-2.5 text-center inline-flex items-center me-2 transition duration-100">
                                <svg class="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                    <path fill-rule="evenodd" d="M11.32 6.176H5c-1.105 0-2 .949-2 2.118v10.588C3 20.052 3.895 21 5 21h11c1.105 0 2-.948 2-2.118v-7.75l-3.914 4.144A2.46 2.46 0 0 1 12.81 16l-2.681.568c-1.75.37-3.292-1.263-2.942-3.115l.536-2.839c.097-.512.335-.983.684-1.352l2.914-3.086Z" clip-rule="evenodd" />
                                    <path fill-rule="evenodd" d="M19.846 4.318a2.148 2.148 0 0 0-.437-.692 2.014 2.014 0 0 0-.654-.463 1.92 1.92 0 0 0-1.544 0 2.014 2.014 0 0 0-.654.463l-.546.578 2.852 3.02.546-.579a2.14 2.14 0 0 0 .437-.692 2.244 2.244 0 0 0 0-1.635ZM17.45 8.721 14.597 5.7 9.82 10.76a.54.54 0 0 0-.137.27l-.536 2.84c-.07.37.239.696.588.622l2.682-.567a.492.492 0 0 0 .255-.145l4.778-5.06Z" clip-rule="evenodd" />
                                </svg>
    ...
    public function updateWithdrawDeviceController(){
        ...
        $withdrawalDate = strtoupper($this->cleanRequest($_POST['withdrawalDate']));
        $returnedByName = strtoupper($this->cleanRequest($_POST['returnedByName']));
        $currentRoomCode = strtoupper($this->cleanRequest($_POST['currentRoomCode']));


        if (empty($withdrawalDate) || empty($returnedByName) || empty($currentRoomCode)) {
            $alert = [
                "type" => "simple",
                "icon" => "warning",
                "title" => "¡Error al Actualizar!",
        ...
        $deviceDataUpdate = [
            ...
            [
                ...
                "db_ValueName" => ":currentRoomCode",
                "db_realValue" => $currentRoomCode
            ],
        ];
        ...
        if($this->updateData("devices", $deviceDataUpdate, $deviceCondition)){
            $alert=[
                ...
                "icon"=>"success",
                "title"=>"¡Operacion Realizada!",
                "text"=>"Dispisitvo retirado actualizado exitosamente",
            ];
        }else{
            $alert=[
                "type"=>"simple",
    ...
    public function withdrawDeviceController(){
        ...
        if (empty($withdrawDate) || empty($returnedByName) || empty($currentRoomCode)) {
            $alert = [
                "type" => "simple",
                "icon" => "warning",
